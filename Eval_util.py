...
def Year_analysis(ret_df,dafult_VaR_year_windows=5,save_=True,file_name=None):
        # 尝试不同的读取方式
        ...
        for _, row in annual_metrics.iterrows():
            print(f"{row['年度']:<6} {row['区间收益率']:<10} {row['最大回撤']:<10} {row['年初到最低点回撤']:<12}  {row['年化波动率']:<10} {row['夏普比率']:<10} {row['卡玛比率']:<10}")
        ...
        # 保存结果到Excel
        if save_ == True:
            filename = file_name if file_name is not None else 'default'
            output_file = f'Analysis_yearly_{filename.split(".")[0]}.xlsx'
        ...
        if len(annual_metrics) > 0:
            # 转换数值列进行统计
            numeric_cols = ['区间收益率', '最大回撤', '年初到最低点回撤', '年化波动率', '夏普比率', '卡玛比率']
            ...
            avg_return = annual_metrics['区间收益率'].mean() * 100
...
def calculate_monthly_metrics_with_var(returns_df, dafult_VaR_year_windows):
    ...
    for (year, month), month_returns in monthly_groups:
        ...
        print(f"该月交易天数: {trading_days}") #test
        # 年化收益率（基于月度收益率）
        if trading_days > 0:
...
def Month_analysis(ret_df, dafult_VaR_year_windows=5, save_=True,file_name=None):
    """
    按月分析净值表现的工具函数
    """
    # 尝试不同的读取方式
    ...
    if save_ == True:
        filename = file_name if file_name is not None else 'default'
        output_file = f'Analysis_monthly_{filename.split(".")[0]}.xlsx'
